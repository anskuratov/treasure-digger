namespace DefaultNamespace
{
	public static class PrefabPath
	{
		public static readonly string ShovelView = "Prefabs/ShovelView";
		public static readonly string GoldWalletView = "Prefabs/GoldWalletView";
		public static readonly string CellView = "Prefabs/CellView";
		public static readonly string GoldBarView = "Prefabs/GoldBarView";
		public static readonly string FieldView = "Prefabs/FieldView";
		public static readonly string GoldBarsSpawnerView = "Prefabs/GoldBarsSpawnerView";
	}
}